...
InputDecoration commonInputDecoration(
  ...
  return InputDecoration(
    ...
    suffixIcon: dateTime != null
        ? dateTime
        : suffixIcon != null
            ? Icon(suffixIcon, color: ColorUtils.colorPrimary, size: 22)
                .onTap(suffixOnTap)
            : null,
    enabledBorder: OutlineInputBorder(
        borderSide: BorderSide(
            style: BorderStyle.solid,
            color: ColorUtils.colorPrimary.withOpacity(0.9)),
...
Widget emptyWidget() {
  return Center(
      child: Image.asset(ic_no_data,
          width: 80, height: 80, color: ColorUtils.colorPrimary));
}
...
String orderStatus(String orderStatus) {
  ...
  } else if (orderStatus == TEST_A) {
    return "Test A"; // for testing purposes
  } else if (orderStatus == TEST_B) {
    return "Test B"; // for testing purposes
  }
...
oneSignalSettings() async {
  if (isMobile) {
    ...
    OneSignal.Notifications.addClickListener((notification) async {
      ...
      if (notId != null) {
        if (!appStore.isLoggedIn) {
          ...
          OrderDetailScreen(orderId: int.parse(notId.toString()))
              .launch(getContext);
        }
      }
    });
    OneSignal.Notifications.addForegroundWillDisplayListener((event) {
      ...
      if (event.notification.additionalData!["type"]
              .toString()
              .contains(ORDER_TRANSFER) ||
          event.notification.additionalData!["type"]
              .toString()
              .contains(ORDER_ASSIGNED)) {
...
void playSoundForDuration() async {
  ...
  try {
    ...
    FlutterRingtonePlayer()
        .play(fromAsset: "assets/ringtone/ringtone.mp3", looping: true);
...
Future<void> openMap(double originLatitude, double originLongitude,
